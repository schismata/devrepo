...
addon_id = 'plugin.video.filmpertutti'
...
def CATEGORIES():
        ...
        addDir2('Ultimi Film','http://www.filmpertutti.click/search/%5BHD%5D/page/1/',1,icon,fanart)
        addDir2('Serie TV','http://www.filmpertutti.click/category/serie-tv/page/1/',3,icon,fanart)
	addDir2('Search TV Shows','http://www.filmpertutti.click/',7,icon,fanart)
	addDir2('Search Movies','http://www.filmpertutti.click/',4,icon,fanart)
...
def GETLINKSNEW(url,name,iconimage):
		...
		matchlink=re.compile('<a href="(.+?)" target="_blank" rel="nofollow">(.+?)</a>').findall(link)
...
def GETEPISODES(url,name):
        ...
        match=re.compile('>\s*(.+?)<a href="(.+?)" target="_blank" rel="nofollow">.+?</a').findall(link)
...
def SEARCH():
    ...
    if len(search_entered)>1:
     try:
    	url = 'http://www.filmpertutti.click/search/' + search_entered
        GETMOVIES(url,name)
     except: pass
