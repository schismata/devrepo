...
addon_id = 'plugin.video.darkstreamtv'
...
def CATEGORIES():
        ...
        addDir2('FILM A-Z','http://www.darkstream.tv/elenco-alfabetico-film-streaming/',1,icon,fanart)
        addDir2('Saghe Complete','http://www.darkstream.tv/elenco-saghe-streaming/',10,icon,fanart)
	addDir2('Serie TV','http://www.darkstream.tv/elenco-serie-tv-streaming/',3,icon,fanart)
	addDir2('Search TV Shows','http://www.darkstream.tv/',7,icon,fanart)
	addDir2('Search Movies','http://www.darkstream.tv/',4,icon,fanart)
...
def GETLINKSNEW(url,name,iconimage):
		...
		matchlink=re.compile('<a href="(.+?)" target="_blank"').findall(link)
...
def GETSEARCHMOVIES(url,name):

		try:link = open_url(url)
		except:link = cloudflare.request(url, mobile=True)
		matchlink=re.compile('<a href="(.+?)" rel="bookmark" title="(.+?)">').findall(link)
		matchlink2=re.compile("<a style='.+?' href='(.+?)' target='_blank'").findall(link)
		
		for url,name in matchlink:
						name=re.sub('&#8211;','',name)
						name=re.sub('&#8217;',' ',name)
						...
						name=re.sub(r'\]',r'', name)
						name=re.sub(r'\/',r'', name)
						addDir2(name,url,2,icon,'')
					
			
def GETSEARCHTV(url,name):
...
def GETEPISODES(url,name):
        ...
        match2=re.compile('<p>(.+?) <a href="(.+?)" target="_blank" rel="nofollow" rel="nofollow">').findall(link)
        ...
        for name,url in match:
			...
			name=re.sub('<strong>','  ',name)
			host=url.split('/')[2].replace('www.','').capitalize()
			addLink(name + "   ..." + host,url,101,icon,'')
            # except:pass
...
def GETMOVIES(url,name):
        ...
        search_entered ='Per i numeri basta inserire 0-9'
        keyboard = xbmc.Keyboard(search_entered, 'Inserire lettera')
        keyboard.doModal()
        ...
        basepage=url
        lpage = basepage + "elenco-film-"+search_entered+"/"
        try:link = open_url(lpage)
        except:link = cloudflare.request(lpage, mobile=True)
...
def SEARCH():
    ...
    if len(search_entered)>1:
     try:
    	url = 'http://www.darkstream.tv/?s=' + search_entered
        GETSEARCHMOVIES(url,name)
     except: pass
...
elif mode==10: GETMOVIES(url,name)
