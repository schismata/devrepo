...
###########################################################################################################################################################
    ...
    for i in range(first,total):
        show=shows[i]
        url="http://www.watchfree.to"+show[0]
        show_title=show[1]
        # show_year=show[2]
        try:
            fav_uri = build_url({'mode': 'add_tv_fav', 'show': show_title,'link': url})
        except:
            fav_uri = build_url({'mode': 'add_tv_fav', 'show': ((show_title).encode('ascii','ignore')),'link':url })
    ...
    dicti=urlparse.parse_qs(sys.argv[2][1:])
    url="http://www.watchfree.to"+dicti['url'][0]
    show=dicti['title'][0]
