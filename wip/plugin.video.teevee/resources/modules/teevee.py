...
def popular_today():
	...
	reg=re.compile('<div class="item"><a href="(.+?)" title=(.+?)">.+?<img')
...
def most_popular():
	...
	soup=bs(html)
	tag=soup.find('div',{'class':'item'})
	reg=re.compile('<a href="(.+?)".+?>(.+?) \((\d+)\)<')
	shows=re.compile('<a href="(.+?)".+?>(.+?) \((\d+)\)<').findall(html) 
...
def get_seasons(show):
	...
	reg=re.compile('<a data-id=".+?" class="season-toggle" href=[\"\'](.+?)[\"\'].+?>Season (\d+)<span')
