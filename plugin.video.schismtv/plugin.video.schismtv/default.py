...
from resources.modules.schismtv import *
import xbmcvfs
...
try:
    ...
    xbmcgui.Dialog().ok("SchisM TV Import Failure", "Failed to import addon.common", "A component needed by SchisM TV is missing on your system", "Please visit www.tvaddons.ag for support")
...
def get_links(url):
    ...
    tag=str(soup.find('div',{'class':'page_content'}))

    reg=re.compile('<a href="/go.php?(.+?)" rel="nofollow"')
    links=re.compile('<a href="(.+?)" rel="nofollow" title=".+?" target="_blank">').findall(html)
    # links=list(re.findall(reg,tag))
    out=[]
    ind=0
    for link in links:
        link = "http://www.watchfree.to"+link
        out+=[link]
        ind=0
    return out
...
def Notify(typeq, box_title, message, times='', line2='', line3=''):
     if box_title == '':
          box_title='SchisM TV Notification'
     if typeq == 'small':
          if times == '':
               times='5000'
...
def add_favourite_show(name, link, thumb):
    with db:
        ...
        data=cur.fetchall()
        if len(data)!=0:
            Notify('small', 'Favourite Already Exists', name + ' already exists in your SchisM TV favourites','')
            return
        
        cur.execute("INSERT INTO Favourite_shows(Link,Title, Thumb) VALUES (?,?, ?);",(link,name,thumb))
        db.commit()
        cur.close()
    Notify('small', 'Added to favourites', name + ' added to your SchisM TV favourites','')
    return
...
if play:
    ...
    for i in range(len(links)):
		...
		try:
			...
			# dp = xbmcgui.DialogProgress()
			# dp.create("SchisM TV: Sorting Links",link)
			dp.update(int(progress), 'Scraping links',link)
			sources.append(link)
			
			if "vidlocker" in link:	one = link
			elif "streamin" in link: two = link
			elif "vidzi" in link: three = link
			elif "bestream" in link: four=link
			elif "vidto" in link: five=link
			elif "thevideo" in link: six=link
			elif "vidspot" in link: seven=link
			elif "vidbull" in link: eight=link
		except:pass    
       ...
       if index>-1:
        ...
        for i in range(len(links)):
         ...
         try:
            ...
            resolved=urlresolver.HostedMediaFile(link).resolve()

            if resolved:
				addon.resolve_url(resolved)
				break
            else:
                Notify('small', 'No stream', link,'')
         except: pass  	
    elif autoplay=='false':
		    ...
		    for i in range(len(links)):
				url=links[i]
				try:
					resolved=urlresolver.HostedMediaFile(one).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",one)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(two).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",two)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(three).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",three)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
						...
						# elif "bestream" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
						# elif "vodlocker" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
					
				try:
					resolved=urlresolver.HostedMediaFile(four).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",four)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(five).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",five)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(six).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",six)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(seven).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",seven)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
				...
				try:
					resolved=urlresolver.HostedMediaFile(eight).resolve()
					dp = xbmcgui.DialogProgress()
					dp.create("SchisM TV: TRYING Links",eight)
						# else: resolved=urlresolver.HostedMediaFile(link).resolve()
					if resolved:
        ...
        # if not resolved:
			...
			# elif "vodlocker" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
			# elif "vidspot" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
			# elif "bestream" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
			# elif "vidbull" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
			

				# elif "vidzi" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "vidto" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "streamin" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "vidspot" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "bestream" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "vidbull" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "thevideo" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
				# elif "thefile" in link: resolved=urlresolver.HostedMediaFile(link).resolve()
...
###########################################################################################################################################################
    ...
    for i in range(first,total):
        ...
        context=[('Show Information', 'XBMC.Action(Info)'), ('Add to SchisM TV favourites','RunPlugin(%s)'%fav_uri)]
        meta=None
        if meta_setting!='false':
            imdb_id=get_imdb(url)
            metaget=metahandlers.MetaData()             
    ...
    for item in episodes:
        ...
        if meta_setting!='false':
			...
			metaget=metahandlers.MetaData()             
			try:
				meta=metaget.get_meta('tvshow', show_title,imdb_id=imdb_id)
...
###########################################################################################################################################################
    ...
    for i in range(total):
        ...
        context=[('Show Information', 'XBMC.Action(Info)'),
                    ('Remove from SchisM TV favourites','RunPlugin(%s)'%del_uri),
                    ('Remove all SchisM TV favourites','RunPlugin(%s)'%del_all)]
        meta=None
        if meta_setting!='false':
