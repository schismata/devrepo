	...
	import urlresolver
        ...
        for i in range(len(links)):
            ...
            if resolved:
                ...
                break
        if not resolved:
            Notify('small', 'No stream', "TESTMESSAGECouldn't resolve item",'')
...
###########################################################################################################################################################
    ...
    for i in range(len(meta)):
        ...
        addon.add_directory({'mode': 'open_season', 'url': 'http://www.watchfree.to' + seasons[i][0],'num':seasons[i][1], 'show': show }, metas, img=metas['cover_url'], fanart=metas['backdrop_url'], total_items=total)
    xbmcplugin.endOfDirectory(addon_handle)
