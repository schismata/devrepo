...
def GETCASACINEMA(url,name):
	...
	# episodes=re.findall(reg,str(tag))
        ...
        match_next=re.compile('<a class="nextpostslink" rel="next" href="(.+?)">').findall(link)
        for url,name in match:
                        ...
                        name=re.sub('&#8211;', '', name)
                        name=re.sub('&#8217;','', name)
                        addDir2(name,url,18,icon,'',icon)
                
        for nextpage in match_next:  
               
                addDir2('[COLOR yellow]Next Page >>[/COLOR]',nextpage,17,icon,'',icon)
def GETCASACINEMALINKS(url,name):
	metaset = selfAddon.getSetting('enable_meta')
	...
	match=re.compile('\s*(.+?)\s*<a href="(.+?)" target="_blank" rel="nofollow" rel="nofollow">').findall(link) 
...
def GETLINK(url,name):
	...
	try:link = open_url(url)
        ...
        # match=re.compile('<p>(.+?) <a href="(.+?)" target="_blank" rel="nofollow">').findall(link)
        # match2=re.compile('.+?>(.+?)\s*<a href="(.+?)" target="_blank" rel="nofollow">').findall(link)      
	match3=re.compile('>\s*(.+?)a\s*href="(.+?)" target="_blank" .+?</a').findall(link) 
...
def SEARCHMOVIES():
    ...
    if len(search_entered)>1:
      try:
			...
			match=re.compile('<a href="(.+?)" title="(.+?)">\s*<img src="(.+?)"').findall(link)[:5]
      ...
      try:
        url = 'http://www.italiaserie.news/?s='+ search_entered
        try:link = open_url(url)
        except:	link = cloudflare.request(url, mobile=True)   
        if 'category/serials/' in url: metaset='false'
        match=re.compile('<a href="(.+?)" title="(.+?)">\s*<img src="(.+?)"').findall(link)[:5]
        ...
        for url,name,img in match:
                        ...
                        addDir2(name + "  ... Italiaserie.news",url,10,img,'',img)
      except:pass 	  
...
elif mode==17: GETCASACINEMA(url,name)
elif mode==18: GETCASACINEMALINKS(url,name)
